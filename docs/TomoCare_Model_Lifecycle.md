...
Current decision — September 19, 2026: Rosa deferred extraction-model evaluation/migration. Retain the existing supported configuration while finishing the demo track. Neither a GPT replacement nor the stable Gemini candidate is validated for TomoCare extraction. Revisit after the portfolio checkpoint, or sooner if a provider retirement/access issue requires action. Accuracy and character quality remain acceptance gates for any future upgrade.
