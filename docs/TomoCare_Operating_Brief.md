...
**Synthetic Veterinary Documents and Demo-Safe Gmail Intake is at Gate 3 validation.** Gate 1 added the polished fictional invoice and exact allowlisted Gmail-to-demo-Storage boundary. Gate 2 carries only that manifest-owned source through deterministic candidate extraction and bounded Verification Intelligence, with one planned human correction for the missing invoice number. Gate 3 keeps correction separate from explicit verification, materializes source-linked Librela, weight, cost, and clinic-status evidence through the shared real product path, exposes that result through Dashboard, Chat, and Voice, and constrains reset to manifest-derived rows and one exact Storage object. Calendar and Messages destinations remain blocked. Broader preventive lifecycle expansion—additional vaccines, annual wellness, annual-lab lifecycle state, preventive screening, reminders, and completion reconciliation—moves to bounded Real-Care work after the portfolio checkpoint unless Rosa explicitly reprioritizes it.
